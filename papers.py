...
def processcitation(citation):
    # Split the authors and year from the citation
    authors_year_part = citation.split('(')
    authors = authors_year_part[0].strip()  # Authors are before the '('
    year = authors_year_part[1][:4] if len(authors_year_part) > 1 else ""  # Year is within parentheses

    # Determine the type based on the outlet (journal or conference)
    if "Proceedings" in citation:
        type = "Conference Article"

        # For conference articles, split based on different patterns
        title = citation.split('.')[1].strip()  # Title is between the first period and the comma
        outlet = citation.split(',')[1].strip()  # Outlet is after the first comma
        pp = citation.split(',')[2].strip() if len(citation.split(',')) > 2 else ""  # Pages if available

        # Volume and issue may not be present in conference citations, so leave empty
        vol_issue = ""
        issue = ""
        article = citation.split(',')[3].strip() if len(citation.split(',')) > 3 else ""  # Article no. if available

    else:
        type = "Journal Paper"

        # For journal papers, use typical split
        title = citation.split('.')[1].strip()  # Title is between the first period and the comma
        outlet = citation.split(',')[1].strip()  # Outlet is after the first comma
        vol_issue_part = citation.split(',')[2].strip().split()  # Volume and issue are together
        vol_issue = vol_issue_part[0]  # Volume is the first part
        issue = vol_issue_part[1] if len(vol_issue_part) > 1 else ""  # Issue if available
        pp = citation.split(',')[3].strip() if len(citation.split(',')) > 3 else ""  # Pages if available
        article = citation.split(',')[4].strip() if len(citation.split(',')) > 4 else ""  # Article no. if available

    # Use the processtitle function to format the title
    title = processtitle(title)

    # Ensure both types return the same number of values
